 ...
 */
public abstract class AbstractJpaTransactionSpring implements JpaTransaction {
    private static final Logger LOG = LoggerFactory.getLogger(AbstractJpaTransactionSpring.class);
    ...
    @Transactional(propagation=Propagation.REQUIRES_NEW,
    		       rollbackFor=RollbackWarning.class)
   	@Override
    public void runInTransaction(Transactable transactable) throws Exception {
        ...
        try { 
            ...
            if ( defaultRollback ) {
                throw new RollbackWarning(); // signal to Spring to rollback.
            }
        } catch (RollbackWarning rt) {
        	// Just throw this up to the caller
        	throw rt;
        } catch (Exception e) {
        	// We got some other exception, wrap it in a RollbackWarning to
        	// signal Spring to rollback the transaction.  The caller will 
        	// need to look at the underlying cause.
            LOG.error("Error running job", e);
            throw new RollbackWarning(e);
        }
    }
}
